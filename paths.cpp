...
            if (q[j] != u) Relax(u, q[j], BuildingDistance(u, q[j]), d);
...
                //	printf("Removed %d bad bids.  %d edges left\n", bids->numDisconnectedBids, numEdgesInBids);
...
                               bids->numBids(), p.remove_dominated_bids ? "non-dominated " : "", numEdgesInBids, p.num_bids);
...
        randomizeParamsWithDistribution(p.param_dists[(int) Param::PATHS], 5, param_mins, param_maxs, params, p);
