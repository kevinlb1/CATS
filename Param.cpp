...
// all distribution-specific parameters are stored in a table as strings, 
// and parsed in the distribution itself
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (argv[i][0] == '-' || argv[i][0] == '/') {
            ...
            // distribution
                ...
                if (i + 1 >= argc || distribution != UNASSIGNED ||
                    dist_dist_file != NULL) {
                    usage();
                    parse_error = true;
                    return;
...
void Param::parseModelFiles()
{
    ...
    while (!modelFilenamesStream.eof()) {
        ...
        if (!stricmp(buf, "-feat_poly")) {
            ...
            if (distIndex == -1 || distIndex == Param::REGIONS ||
...
char *Param::outputSettings(bool tofile)
{
    ...
    if (param_dists[distribution]) {
        sprintf(param_dist_str, "Parameters generated from distribution in file \"%s\".", 
                param_dists[distribution]->filename);
    ...
    if (tofile) {
        sprintf(output_settings_buffer, 
                "%% Goods: %d; Bids: %d; Distribution: %s; Runs: %d; %s%s%sSeed: %d", 
                num_goods, num_bids, dname[distribution], num_runs, dist_dist_str, 
                param_dist_str, feat_poly_str, random_seed);
    }
    else {
        sprintf(output_settings_buffer, 
                "Goods: %d\nBids: %d\nDistribution: %s\nRuns: %d\n%s%s%sSeed: %d\n", 
