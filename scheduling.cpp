        ...
        do {
            ...
            for (start = 0; start < p.num_goods; start++) {
                if ((start + l - 1 <= new_d) && (cur_max_deadline < start + l - 1)) {
                    bids->addXor(makeBid(start, start + l - 1, dev * pow((double) l, 
                                    1 + additivity) * d_1 / new_d));
                }
            }

            cur_max_deadline = new_d;

            if (cur_max_deadline == p.num_goods - 1) {
                break;
            }
...
void Scheduling::randomizeParams(Param &p)
{
    if (p.param_dists[(int) Param::SCHEDULING] != NULL) {
        ...
        randomizeParamsWithDistribution(p.param_dists[(int) Param::SCHEDULING], 4, 
                sched_param_mins, sched_param_maxs, params, p);
