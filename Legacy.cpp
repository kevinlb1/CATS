...
Legacy::Legacy(Param &p, char * argv[], int argc)
{
    ...
    if (num_goods_type == NORMAL_GOODS || pricing_type == NORMAL_PRICE) {
        norm = new Normal(p);
    }
}
...
void Legacy::randomizeParams(Param &p)
{
    if (p.param_dists[(int) p.distribution] != NULL) {
        ...
        case Param::L1:
            ...
            randomizeParamsWithDistribution(p.param_dists[(int) p.distribution], 1, 
                    param_mins + 1, param_maxs + 1, params, p);
        ...
        case Param::L8:
            ...
            randomizeParamsWithDistribution(p.param_dists[(int) p.distribution], 2, 
        ...
        case Param::L5:
            ...
            randomizeParamsWithDistribution(p.param_dists[(int) p.distribution], 4, 
                    param_mins, param_maxs, params, p);
            mu_goods = params[0];
            mu_price = params[1];
...
BidSet *Legacy::generate(Param &p)
{
    ...
    for (int t = 1; bidset->numBids() < p.num_bids; t++) {
        ...
        Bid *bid = generateBid(num, p.num_goods, price); 
        // this selects num goods uniformly without replacement
...
char *Legacy::outputSettings(bool tofile)
{
    ...
    sprintf(output_buffer, "%s%s%s%s%s%s.\n", buffer1, (tofile || !buffer2[0] ? "" : "\n  "), 
            buffer2, buffer3, (tofile || !buffer4[0] ? "" : "\n  "), buffer4);
