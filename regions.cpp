...
        if (location[loc1].neighbor[i] == loc2) return true;
...
    if (b->num_items >= num_goods) return;
...
        for (g = 0; rand_num > sum + location[g].pn && g < num_goods; g++) // this relies on sum_i location[i].pn == 1 
...
            strlen(buffer5) + strlen(buffer6) + strlen(buffer7) + strlen(buffer8) + strlen(buffer9) + strlen(buffer10) +

            strlen(buffer11) + strlen(buffer12)/*+strlen(buffer13)+strlen(buffer14)+strlen(buffer15)*/ + 20;
...
            buffer7, buffer8, buffer9, buffer10, buffer11, buffer12/*,buffer13,buffer14,buffer15*/);
...
            randomizeParamsWithDistribution(p.param_dists[(int) effectiveDist], 6, param_mins, param_maxs, params, p);

        }
        else

            randomizeParamsWithDistribution(p.param_dists[(int) effectiveDist], 7, param_mins, param_maxs, params, p);
